...
from config import *
from state import initial_score
...
def ticker_msg(ev, bat, bowl, runs):
    ...
    if ev == "LBWCHECK": return f"LBW CHECK! {bat or 'Batter'} under review"
    if ev == "RUNOUTCHECK": return f"RUN OUT CHECK! {bat or 'Batter'} under review"
    if ev == "DRS": return f"DRS REVIEW! {bat or 'Player'} involved"
...
def event_text(event):
    fields = [
        ...
        event.get("wicketDesc", ""),
    ]
    return strip_html(" ".join(str(value or "") for value in fields))

def review_player_from_event(event, fallback=""):
    for key in ["player_fullname", "player", "batter", "batsman", "striker", "p1", "n"]:
        value = str(event.get(key) or "").strip()
        if value:
            return value

    c1 = str(event.get("c1") or "")
    if " to " in c1:
        return c1.split(" to ", 1)[1].strip()
    return fallback

def detect_review_event(event, fallback_player=""):
    text = event_text(event)
    if not text:
        return "", "", ""

    text_lower = text.lower()
    has_check = re.search(r"\b(check|checking|checked|review|reviews|reviewed|drs|referred|referral|third\s+umpire|upstairs|decision\s+review)\b", text_lower)
    if not has_check:
        return "", "", ""

    player = review_player_from_event(event, fallback_player)
    if re.search(r"\blbw\b|leg\s+before", text_lower):
        return "LBWCHECK", "LBW CHECK", player
    if re.search(r"\brun\s*-?\s*out\b|runout", text_lower):
        return "RUNOUTCHECK", "RUN OUT CHECK", player
    if re.search(r"\bdrs\b|decision\s+review|player\s+review|review\b", text_lower):
        return "DRS", "DRS REVIEW", player
    return "", "", ""

def set_review_event(kind, label, player=""):
    latest_score["event"] = kind
    latest_score["reviewEvent"] = label
    latest_score["reviewText"] = f"{label}: {player}" if player else label
    latest_score["reviewPlayer"] = player
    latest_score["ticker"] = ticker_msg(kind, player, latest_score.get("lastBowler", ""), "")

def clear_review_event():
    latest_score["reviewEvent"] = ""
    latest_score["reviewText"] = ""
    latest_score["reviewPlayer"] = ""

def set_latest_commentary_text(event):
    text = event_text(event)
    if text:
        latest_score["commentaryText"] = text
...
def result_from_text(text):
    ...
    if re.search(r"\b(match\s+closed|time\s+to\s+say\s+goodbye|signing\s+off|as\s+a\s+consequence\s+of\s+this\s+result|post[-\s]?match|match\s+summary)\b", clean, re.I):
        return "Match closed"
...
def winner_from_result(result):
    ...
    if not result or re.search(r"\b(match\s+tied|no\s+result|abandoned|called\s+off)\b", result, re.I):
        return ""

    for team in [
        latest_score.get("team1", ""),
        latest_score.get("team2", ""),
        latest_score.get("battingTeam", ""),
        latest_score.get("bowlingTeam", ""),
    ]:
        if not team:
            continue
        pattern = rf"\b{re.escape(team)}\b.*?\b(?:won|wins|win|beat|beats|defeated|defeats)\b"
        if re.search(pattern, result, re.I):
            return team

    match = re.search(r"\b([A-Za-z][A-Za-z0-9 .&'()-]{1,60}?)\s+(?:won|wins|win|beat|beats|defeated|defeats)\b", result, re.I)
    if match:
        return team_from_abbr(match.group(1).strip())

    if re.search(r"\bmatch\s+closed\b", result, re.I):
        inferred = infer_match_result()
        if inferred and inferred != result:
            return winner_from_result(inferred)
...
def toss_from_text(text):
    ...
    if not clean:
        return ""
    patterns = [
        r"([A-Za-z .&'-]+?)\s+(?:won|wins|have won|has won)\s+(?:the\s+)?toss\s*(?:,|and)?\s*(?:have\s+|has\s+|will\s+)?(?:elects|elected|opts|opted|chooses|chosen|chose|decides|decided)?\s*(?:to\s+)?(bat|bowl|field)",
        ...
        r"([A-Za-z .&'-]+?)\s+(?:will|to)\s+(bat|bowl|field)\s+first\s+after\s+winning\s+the\s+toss",
    ]
    for pattern in patterns:
        match = re.search(pattern, clean, re.I)
        if match:
...
def toss_from_value(value):
    if not value:
        return ""
    if isinstance(value, str):
        return toss_from_text(value)
    if isinstance(value, dict):
        winner = (
            value.get("winner") or value.get("team") or value.get("tossWinner")
            or value.get("wonBy") or value.get("won_by") or value.get("name") or ""
        )
        decision = (
            value.get("decision") or value.get("elected") or value.get("choice")
            or value.get("opted") or value.get("selected") or ""
        )
        parsed = toss_from_text(json.dumps(value, ensure_ascii=False))
        if parsed:
            return parsed
        if winner:
            decision_text = str(decision or "").lower()
            if re.search(r"bat", decision_text):
                return f"{toss_team_name(winner)} won toss, batting"
            if re.search(r"bowl|field", decision_text):
                return f"{toss_team_name(winner)} won toss, bowling"
            return f"{toss_team_name(winner)} won toss"
    return toss_from_text(str(value))
...
def set_finished_match(event):
    ...
    result = event.get("c", "")
    winner_name = winner_name or winner_from_result(result)

    latest_score["matchResult"] = result
    latest_score["winnerTeam"] = winner_name
...
def set_finished_text(text):
    ...
    winner_name = winner_from_result(result)
    latest_score["matchResult"] = result
    latest_score["winnerTeam"] = winner_name
...
def set_match_closed(text="Match closed"):
    ...
    winner_name = winner_from_result(text)
...
def process_events(events):
    ...
    for event in sorted(events, key=lambda x: x.get("id", 0)):
        ...
        set_latest_commentary_text(event)
        review_kind, review_label, review_player = detect_review_event(event, latest_score.get("lastBatter", ""))
        if review_kind:
            set_review_event(review_kind, review_label, review_player)
        ...
        if etype == "b":
            ...
            review_kind, review_label, review_player = detect_review_event(event, bat_short)
            if review_kind and not ev:
                set_review_event(review_kind, review_label, review_player)
            else:
                clear_review_event()
